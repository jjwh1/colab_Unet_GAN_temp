...
# AOT-GAN   https://github.com/researchmm/AOT-GAN-for-Inpainting/blob/master/src/data/dataset.py
...
class InpaintDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        large_mask = cv2.imread(large_mask_path, cv2.IMREAD_GRAYSCALE)  # cv2.IMREAD_GRAYSCALE 옵션을 빼면 BGR 형식으로 읽어 오류가 날 수 있음
        ...
        if large_mask is None:
            raise FileNotFoundError(f"Could not read ground truth at {large_mask_path}")
        ...
        return input_tensor, gt, mask, large_mask
