...
from models import TwostagendGenerator, DeepFillDiscriminator
...
def train_gan_epoch(generator, discriminator1,discriminator2, dataloader, optimizer_g, optimizer_d, device):  # 한 에포크 학습 정의
    generator.train()
    ...
    for images, gts, masks,largemasks in progress_bar:
        ...
        fake_images1, fake_images2,cr,_ = generator(images,masks,largemasks)
        real_output = discriminator1(gts,masks) # dis의 output: (batch_size, 1) 형태의 출력 텐서
        ...
        fake_output = discriminator1(fake_images2.detach(),masks) # dis의 output: (batch_size, 1) 형태의 출력 텐서
        ...
        real_output2 = discriminator2(gts, masks)  # dis의 output: (batch_size, 1) 형태의 출력 텐서
        # 네트워크에서 생성된 값이 아니라 데이터셋에서 직접 가져온 Ground Truth 이미지이기 때문에 이 데이터는 모델의 그래디언트 업데이트에 영향을 주는 학습 파라미터와 연결된 계산 그래프에 속하지 않음
        # 따라서, 이미 계산 그래프와 분리되어 있으므로 detach()가 필요하지 않습니다.
        # gts는 외부에서 불러온 이미지 (고정된 것이고, 변하면 안됨)이니 그라디언트 자체가 없음
        cr_output = discriminator2(cr.detach(), masks)  # dis의 output: (batch_size, 1) 형태의 출력 텐서
        ...
        cr_loss_real = torch.mean(F.relu(1. - real_output2))
        #  Fake image loss
        cr_loss_fake = torch.mean(F.relu(1. + cr_output))
        cr_loss = 0.5*cr_loss_real + 0.5*cr_loss_fake

        (d_loss+cr_loss).backward()
        optimizer_d.step()
        ...
        g_loss_adv = -torch.mean(discriminator1(fake_images2, masks))

        cr_loss_adv = -torch.mean(discriminator2(cr, masks))

        g_loss_pixel1 = nn.L1Loss()(fake_images1, gts)
        g_loss_pixel = nn.L1Loss()(fake_images2, gts)

        cr_loss_pixel = nn.L1Loss()(cr, gts)


        g_loss = 1.5*g_loss_adv+ 1.5*g_loss_pixel1+1.5*g_loss_pixel +0.5*cr_loss_adv+0.5*cr_loss_pixel
        g_loss.backward()
        optimizer_g.step()
...
def validate_epoch(generator, discriminator1, discriminator2,dataloader, device, writer, epoch,save_dir=None):
    ...
    with torch.no_grad():
        for i, (images, gts, masks, largemasks) in enumerate(dataloader):
            ...
            psnr(fake_images2, gts)  # 계속 누적됨 (각 배치당 psnr이 누적(배치size로 평균처리)돼서 한 epoch를 채우면 밑에서 compute를 통해 반환)
            ssim(fake_images2, gts)  # 계속 누적됨 (각 배치당 ssim이 누적(배치size로 평균처리)돼서 한 epoch를 채우면 밑에서 compute를 통해 반환)
...
def load_checkpoint(checkpoint_path, generator, discriminator1,discriminator2, optimizer_g, optimizer_d):
    checkpoint = torch.load(checkpoint_path)
    generator.load_state_dict(checkpoint["generator_state_dict"])
    discriminator1.load_state_dict(checkpoint["discriminator1_state_dict"])  # 수정됨
    discriminator2.load_state_dict(checkpoint["discriminator2_state_dict"])  # 수정됨
    optimizer_g.load_state_dict(checkpoint["optimizer_g_state_dict"])
    ...
    g_adv_loss = checkpoint["g_adv_loss"]
    d_loss = checkpoint["d_loss"]
    return generator, discriminator1,discriminator2, optimizer_g, optimizer_d, epoch, g_loss, g_l2_loss, g_adv_loss, d_loss



def main():
    # Paths
    save_dir = "/content/drive/MyDrive/inpaint_result/CASIA_Lamp/CRFill_lr_00010001_fold2_colab/db2_train"
    ...
    checkpoint_path = "/content/drive/MyDrive/inpaint_result/CASIA_Lamp/CRFill_lr_00010001_fold2_colab/db2_train/checkpoint_epoch_228.tar"  # 불러올 시 마지막 저장된 pth파일 경로 입력!!
    ...
    # Models and losses
    generator = TwostagendGenerator().to(device)
    discriminator1 = DeepFillDiscriminator().to(device)
    discriminator2 = DeepFillDiscriminator().to(device)

    # Optimizers
    optimizer_g = optim.Adam(generator.parameters(), lr=lr_g, betas=(0.5, 0.999))
    optimizer_d = optim.Adam(list(discriminator1.parameters())+list(discriminator2.parameters()), lr=lr_d, betas=(0.5, 0.999))
    ...
    if checkpoint_path:
        generator, discriminator1,discriminator2, optimizer_g, optimizer_d, start_epoch, g_loss, g_l2_loss,g_adv_loss, d_loss \
            = load_checkpoint(checkpoint_path, generator, discriminator1,discriminator2, optimizer_g, optimizer_d)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        g_loss, g_l2_loss, g_adv_loss,  d_loss = train_gan_epoch(
            generator=generator, discriminator1=discriminator1, discriminator2=discriminator2,             # model
        ...
        val_g_loss, val_g_l2_loss, val_g_adv_loss, val_d_loss, psnr, ssim = validate_epoch(
            generator=generator,
            discriminator1=discriminator1,discriminator2=discriminator2,
