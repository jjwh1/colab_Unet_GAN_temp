...
from models import RR_inpaint_model_minus_trans, Discriminator
...
from loss import PerceptualLoss, d_logistic_loss, g_nonsaturating_loss
import kornia
...
def train_gan_epoch(generator,discriminator,dataloader, optimizer_g,optimizer_d, perceptualloss,device):  # 한 에포크 학습 정의
    generator.train()
    ...
    for images, gts, masks,largemasks in progress_bar:
        ...
        noise, fake_images = generator(images,masks)

        real_validity = discriminator(gts).mean()
        fake_validity = discriminator(fake_images.detach()).mean()
        ...
        # g_noise_loss_pixel = nn.L1Loss()(noise*largemasks, masks*largemasks)
        g_noise_loss_pixel = nn.L1Loss()(noise*largemasks, largemasks)

        g_perceptual_loss = perceptualloss(fake_images, gts)

        out= fake_images*largemasks + images*(1-largemasks)

        g_loss_adv = g_nonsaturating_loss(discriminator(out))

        g_loss = g_loss_pixel + g_noise_loss_pixel+ 0.001 * g_loss_adv + 0.1 *g_perceptual_loss
...
def validate_epoch(generator, discriminator, dataloader, perceptualloss, device, writer, epoch,save_dir=None):
    generator.eval()
...
def main():
    ...
    # Paths
    save_dir = "/content/drive/MyDrive/inpaint_result/CASIA_Distance/Decontamination_noiseloss_largemasks_lr_0000500001_fold2_colab/db2_train"
    ...
    # Models and losses
    generator = RR_inpaint_model_minus_trans().to(device)
