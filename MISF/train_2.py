...
from models import InpaintGenerator, Discriminator
...
def train_gan_epoch(generator, discriminator, dataloader, optimizer_g, optimizer_d, device, styleloss, perceptualloss):  # 한 에포크 학습 정의
    generator.train()
    discriminator.train()
    ...
    for inputs, gts, masks,largemasks in progress_bar:
        ...
        g_loss_adv = nn.BCELoss()(fake_output, torch.ones_like(fake_output).to(device))
...
def validate_epoch(generator, discriminator, dataloader, device, writer,styleloss, perceptualloss, epoch,save_dir=None):
    generator.eval()
...
def main():
    # Paths
    save_dir = "/content/drive/MyDrive/inpaint_result/CASIA_Lamp/MISF_lr_000100001_fold2_colab/db2_train"
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        g_loss, g_l2_loss, g_adv_loss,  d_loss = train_gan_epoch(
            ...
            styleloss = styleloss, perceptualloss=perceptualloss           # loss
            )
        ...
        val_g_loss, val_g_l2_loss, val_g_adv_loss, val_d_loss, psnr, ssim = validate_epoch(
            ...
            writer=writer,
            epoch=epoch,
            styleloss=styleloss, perceptualloss=perceptualloss,
            save_dir=save_dir)
