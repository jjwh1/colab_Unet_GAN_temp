...
from models import DietPepsiPlusNet, Discriminator
...
def train_gan_epoch(generator, discriminator, dataloader, optimizer_g, optimizer_d, current_epoch, max_epoch , device):  # 한 에포크 학습 정의
    generator.train()
    discriminator.train()
    ...
    for images, gts, masks,largemasks in progress_bar:
        ...
        fake_images, coarse_images = generator(images,masks)
        ...
        g_loss_adv = -torch.mean(discriminator(fake_images))

        g_loss_pixel = nn.L1Loss()(fake_images, gts)
        coarse_loss_pixel = nn.L1Loss()(coarse_images, gts)
        weighted_coarse_loss = (1 - alpha) * coarse_loss_pixel  # [Modified]

        g_loss = 0.1 * g_loss_adv+ 10*g_loss_pixel+ 5*weighted_coarse_loss
        g_loss.backward()
        optimizer_g.step()
...
def validate_epoch(generator, discriminator, dataloader, device, writer, epoch,save_dir=None):
    generator.eval()
    ...
    with torch.no_grad():
        for i, (images, gts, masks, largemasks) in enumerate(dataloader):
            ...
            g_loss = 0.1 * g_loss_adv + 10 * g_loss_pixel
...
def main():
    # Paths
    save_dir = "/content/drive/MyDrive/inpaint_result/CASIA_Lamp/PEPSI++_lr_00010004_fold2_colab/db2_train"
    ...
    # Models and losses
    generator = DietPepsiPlusNet().to(device)
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        g_loss, g_l2_loss, g_adv_loss,  d_loss = train_gan_epoch(
            ...
            current_epoch=epoch,  # [Modified]
            max_epoch=num_epochs,  # [Modified]
        # optim
