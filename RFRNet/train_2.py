...
from loss2 import PerceptualLoss
...
def train_gan_epoch(generator, dataloader, optimizer_g, device, perceptualloss):  # 한 에포크 학습 정의
    ...
    epoch_g_loss, epoch_g_l2_loss, epoch_g_adv_loss = 0.0, 0.0, 0.0
    total_samples = 0
    ...
    for images, gts, masks, largemasks in progress_bar:
        ...
        # masks = masks.expand(-1, 3, -1, -1).contiguous()
        largemasks = largemasks.expand(-1, 3, -1, -1).contiguous()

        # fake_images,_ = generator(images,masks)
        fake_images,_ = generator(images,largemasks)
        ...
        # g_loss_pixel = nn.L1Loss()(fake_images * (1 - masks), gts * (1 - masks)) + 6 * nn.L1Loss()(fake_images * masks, gts * masks)
        g_loss_pixel = nn.L1Loss()(fake_images * (1 - largemasks), gts * (1 - largemasks)) + 6 * nn.L1Loss()(fake_images * largemasks, gts * largemasks)

        g_perceptual_loss, g_style_loss= perceptualloss(fake_images, gts)

        g_loss =  g_loss_pixel + 180*g_style_loss + 0.1*g_perceptual_loss
        g_loss.backward()
        optimizer_g.step()
...
def validate_epoch(generator, dataloader, device, writer, perceptualloss, epoch,save_dir=None):
    ...
    with torch.no_grad():
        for i, (images, gts, masks, largemasks) in enumerate(dataloader):
            ...
            g_loss_pixel = nn.L1Loss()(fake_images * (1 - largemasks), gts * (1 - largemasks)) + 6 * nn.L1Loss()(
                fake_images * largemasks, gts * largemasks)
    ...
    return val_g_loss, val_g_l2_loss, val_g_adv_loss, psnr_value, ssim_value
...
def main():
    # Paths
    save_dir = "/content/drive/MyDrive/inpaint_result/CASIA_Lamp/RFRNet_largemask_lr_0001_fold2_colab/db2_train"
    ...
    # save_dir = r"D:\inpaint_result\CASIA_Lamp\HiFill_lr_000100001_fold2_py/db2_train"
    # writer = SummaryWriter(os.path.join(save_dir, 'SR_Stage_4%s' % datetime.now().strftime("%Y%m%d-%H%M%S")))
    #
    ...
    # train_large_mask_paths = r"D:\mask\CASIA_Lamp\algorithm\450to50000_174x174padding_if_gac1_4000_algorithm\db2_test_layer12_0.3_only_mask_h2.8_w3_trainset"  # List of large_mask paths
    #
    ...
    # val_large_mask_paths = r"D:\mask\CASIA_Lamp\algorithm\450to50000_174x174padding_if_gac1_4000_algorithm\db2_test_layer12_0.3_only_mask_h2.8_w3_validset"  # List of large_mask paths

    results_path = os.path.join(save_dir, "metrics.csv")

    os.makedirs(save_dir, exist_ok=True)

    checkpoint_path = "/content/drive/MyDrive/inpaint_result/CASIA_Lamp/RFRNet_largemask_lr_0001_fold2_colab/db2_train/checkpoint_epoch_137.tar"  # 불러올 시 마지막 저장된 pth파일 경로 입력!!
    ...
    with open(results_path, mode='w', newline='') as csvfile:
        ...
        writer_csv.writerow([
            "epoch", "g_loss", "g_l2_loss", "g_adv_loss",
            "val_g_loss", "val_g_l2_loss", "val_g_adv_loss",
            "psnr", "ssim"
        ])
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        g_loss, g_l2_loss, g_adv_loss = train_gan_epoch(
        ...
        val_g_loss, val_g_l2_loss, val_g_adv_loss, psnr, ssim = validate_epoch(
            generator=generator,

            dataloader= val_loader,
            device=device,
            writer=writer,
            epoch=epoch, perceptualloss=perceptualloss,
            save_dir=save_dir)
        ...
        print(
            f"Epoch [{epoch + 1}/{num_epochs}], G_Loss: {g_loss:.4f}, g_l2_Loss: {g_l2_loss:.4f}")
        print("----------validation-----------")
        print(
            f"val_g_loss: {val_g_loss:.4f}, val_g_l2_loss: {val_g_l2_loss:.4f} ,val_g_adv_loss: {val_g_adv_loss:.4f} ,PSNR: {psnr:.4f}, SSIM: {ssim:.4f} ")

        with open(results_path, mode='a', newline='') as csvfile:
            writer_csv = csv.writer(csvfile)
            writer_csv.writerow([
                epoch + 1, g_loss, g_l2_loss, g_adv_loss,
                val_g_loss, val_g_l2_loss, val_g_adv_loss,
                psnr, ssim
            ])
