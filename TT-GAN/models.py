...
class GatedGenerator(nn.Module):
    def __init__(self, in_channels=4, out_channels=3, latent_channels=32):
        ...
        self.refine_conv = nn.Sequential(  # TT-Unet_GAN_D_100x100 그림에서 위 branch dilated conv 부분까지
            ...
            GatedConv2d(latent_channels * 4, latent_channels * 4, 3, 1, 4, dilation=4), # 유지
            GatedConv2d(latent_channels * 4, latent_channels * 4, 3, 1, 8, dilation=8), # 유지
            GatedConv2d(latent_channels * 4, latent_channels * 4, 3, 1, 16, dilation=16) # 유지
        )
    ...
    def forward(self, image, mask, large_mask):
        ...
        # Refinement
        second_masked_img = image * (1-mask) + first_out * mask
        second_in = torch.cat([second_masked_img, mask], dim=1)  # 애초에 mask랑 concat해서 들어가지 않으면 gated conv를 할 수 없으니 concat함.
