...
class TransposeConv2dLayer(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, pad_type='zero',
                 activation='relu', norm='bn', sn=False, scale_factor=2):
...
class GatedConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, pad_type='reflect',
                 activation='lrelu', norm='bn', sn=False):
...
class TransposeGatedConv2d(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, padding=0, dilation=1, pad_type='reflect',
                 activation='elu', norm='bn', sn=True, scale_factor=2):
...
class PCAttention(nn.Module):
    def __init__(self, channels):
        ...
        # Pixel Attention: 1x1 Convolution + Sigmoid
        self.pixel_attention = nn.Sequential(
            nn.Conv2d(channels, channels, kernel_size=1),
            nn.Sigmoid()
        )
        # Channel Attention: SENet 방식
        self.global_avg_pool = nn.AdaptiveAvgPool2d(1)  # Global Average Pooling
        self.fc1 = nn.Linear(channels, channels // 16)  # Reduce to C/16
        self.relu = nn.ReLU()
        self.fc2 = nn.Linear(channels // 16, channels)  # Expand back to C
        self.sigmoid = nn.Sigmoid()

    def forward(self, x):
        # Pixel Attention
        pa_weights = self.pixel_attention(x)  # Generate pixel-wise weights
        x = x * pa_weights  # Apply pixel-wise attention

        # Channel Attention
        b, c, _, _ = x.size()
        ca_weights = self.global_avg_pool(x).view(b, c)  # Global average pooling # nn.Linear 는 2차원만 받으니 굳이 view 사용
        ca_weights = self.fc1(ca_weights)  # Fully connected layer 1
        ca_weights = self.relu(ca_weights)  # ReLU activation
        ca_weights = self.fc2(ca_weights)  # Fully connected layer 2
        ca_weights = self.sigmoid(ca_weights).view(b, c, 1, 1)  # Reshape to [B, C, 1, 1]
        x = x * ca_weights  # Apply channel-wise attention
