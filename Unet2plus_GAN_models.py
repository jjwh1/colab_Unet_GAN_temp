...
class UNet_2Plus(nn.Module):
    ...
    def __init__(self, in_channels=4, n_classes=3, feature_scale=4, is_deconv=True, is_batchnorm=True, is_ds=False):   # segmentation 할 때나 is_ds=True로 설정
...
class Discriminator(nn.Module):
    def __init__(self, in_channels=3):
        ...
        self.model = nn.Sequential(
            ...
            # 이거 224x224 기준이면 (batch_size, 1, 24, 24) 로 최종 shape 나옴. 일반적인(이상적인) Discriminator는 스칼라 하나만 뱉는거니 (batch_size,1,1,1)이 맞긴한듯? 이것도 틀린건 아닌듯함. 다른 부분도 보겠다는 거니.....
    ...
    def forward(self, x):
        return self.model(x).view(-1, 1).squeeze(1)  # view까지 한 결과 (Batch * 24 * 24, 1) -> 최종 shape : (Batch * 24 * 24,)