...
from modules import ResBlock, ConvBlock, PCBlock

# https://github.com/birdortyedi/vcnet-blind-image-inpainting/blob/main/engine/trainer.py
# https://github.com/shepnerd/blindinpainting_vcnet/blob/master/tensorflow_version/net/model.py (참고만)
class MPN(nn.Module):
...
class RIN(nn.Module):
    def __init__(self, base_n_channels=32, neck_n_channels=128):
...
class Discriminator(nn.Module):
    def __init__(self, base_n_channels=64):
        ...
        self.features_to_prob = nn.Sequential(
            nn.Linear(8 * base_n_channels * 7 * 7, 1)
        )
