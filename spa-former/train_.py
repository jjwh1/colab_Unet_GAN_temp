...
def train_gan_epoch(generator, discriminator, dataloader, perceptualloss,styleloss, optimizer_g, optimizer_d, device):  # 한 에포크 학습 정의
    generator.train()
    discriminator.train()
    ...
    for images, gts, masks,largemasks in progress_bar:
        ...
        g_style_loss = styleloss(fake_images, gts)
        g_perceptual_loss = perceptualloss(fake_images, gts)
        g_loss_pixel = nn.L1Loss()(fake_images, gts)


        g_loss = 0.1 * g_loss_adv+ g_loss_pixel +250*g_style_loss+0.1*g_perceptual_loss
        g_loss.backward()
        optimizer_g.step()
...
def validate_epoch(generator, discriminator, perceptualloss,styleloss,dataloader, device, writer, epoch,save_dir=None):
    generator.eval()
...
def main():
    # Paths
    save_dir = "/content/drive/MyDrive/inpaint_result/CASIA_Distance/spa-former_batch4_lr_0000500005_fold2_colab/db2_train"
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        g_loss, g_l2_loss, g_adv_loss,  d_loss = train_gan_epoch(
            ...
            perceptualloss=perceptualloss, styleloss=styleloss,
